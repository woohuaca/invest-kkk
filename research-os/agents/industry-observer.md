...
- 二级市场观察源，例如朗新能源研究院「观价」的省级现货市场数据洞察、REAI Lab「从图说起」的产业图解；必须标注 `source_url`，并回溯交易中心、监管机构、官方数据或原始数据源验证关键数值。

## 处理流程

1. 记录事实：来源、日期、事件和原始表述。
2. 区分来源等级：官方或一手资料可以形成高置信事实，二级观察源只能形成待验证信号。
3. 判断信号：该事实是否影响需求、供给、瓶颈、控制点、定价权或数据优势。
4. 标注层级：发电、输电、配电、调度、EMS、虚拟电厂、运维知识。
5. 评估强度：判断信号是弱、中、强。
6. 评估置信度：说明判断是否有足够证据。
7. 输出下一步动作：需要继续跟踪、更新洞察卡片，还是更新假设台账。
